...
class ScoreEntryUpdateRequest(BaseModel):
    """Used to update an existing score-entry assessment."""
...
async def create_score_entry(data: ScoreEntryRequest, db=Depends(get_db)):
    ...
    # 1. Create Question documents (use q{number} _id for eval matching)
    question_docs = []
    ...
    # 2. Create Student + Evaluation documents
    student_ids = []
    all_evaluations = []
    ...
    for s in data.students:
        ...
        await db.students.update_one(
            {"_id": student_id}, {"$set": student_doc}, upsert=True
        )
    ...
    avg_score = (
        round(total_marks_scored / num_students, 1) if num_students > 0 else 0.0
    )
    ...
    await db.assessments.update_one(
        {"_id": assessment_id}, {"$set": assessment_doc}, upsert=True
    )
...
async def get_score_entry(id: str, db=Depends(get_db)):
    ...
    for s in students:
        evals = await db.evaluations.find(
            {"assessmentId": id, "studentId": s["_id"]}
        ).to_list(100)
...
async def update_score_entry(id: str, data: ScoreEntryUpdateRequest, db=Depends(get_db)):
    """Update scores for an existing score-entry assessment. Deletes old evals and replaces."""
    assessment = await db.assessments.find_one({"_id": id})
    if not assessment:
        raise HTTPException(status_code=404, detail="Assessment not found")
