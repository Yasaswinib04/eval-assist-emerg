...
async def get_questions(id: str, db=Depends(get_db)):
    try:
        ...
        if assessment and assessment.get("questionsImages") and len(assessment.get("questionsImages", [])) > 0:
        ...
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Questions error: {e.__class__.__name__}: {str(e)[:200]}")
