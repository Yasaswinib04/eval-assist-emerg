...
const STATUS_LABEL = {
  ...
  mou_drafted: '📜 MOU drafted', signed: '✍️ Signed', opted_out: '🚫 Opted out',
};
...
function renderPane() {
  ...
  if (activeTab === 'dossier') {
    ...
    if (!s.drafts) {
      ...
      pane.innerHTML = `
        ${approved ? '' : `<div class="gate"><b>🧑‍⚖️ HUMAN REVIEW GATE</b> — Edit below, then approve. Nothing sends without you.</div>`}
        ${paramsPanel}
    ...
    pane.innerHTML = `
      ${stopped ? `<div class="gate">🛑 <b>Stop condition active</b> — the school responded. All remaining follow-ups halted automatically.</div>`
                : `<div class="btnrow">
                     ${next ? (nextDrafted
                       ...
                       : `<button onclick="prepareFollowup()">✍️ Draft follow-up ${n + 1}/7 now — from the live conversation</button>`) : ''}
                   </div>
      ...
      ${fps.map((f, i) => `<div class="followup ${i < n ? 'sent' : ''}">
        <div class="fh"><span>Day +${f.day_offset} · ${esc(f.channel)} ${i < n ? '· ✅ sent' : (f.message ? '· 📝 drafted — awaiting review' : '· 🗓️ planned')}</span><span class="angle">${esc(f.angle)}
    ...
    pane.innerHTML = `
      <div class="btnrow">
        ...
        ${s.deck_path ? `<a href="${s.deck_path}" target="_blank"><button class="secondary">🖥️ Open deck</button></a>
    ...
    pane.innerHTML = `
      ...
      : `<div class="gate"><b>⚠️ HUMAN & LEGAL REVIEW GATE</b> — DRAFT requires human + legal review before external share (non-negotiable).</div>
...
function renderCRM() {
  ...
  el.innerHTML = `
    <div class="detail-head"><h2>📊 CRM — ${esc(kitLabel)}</h2>
      <a href="/api/export.csv?kit=${encodeURIComponent(activeKitName || '')}"><button class="secondary">⬇️ Export CSV (opens in Google Sheets)</button></a></div>
...
async function prepareFollowup() {
  toast('✍️ Drafting this touch from the live conversation — sent touches, replies, meeting notes… <span class="spin">⏳</span>', 60000);
...
function openNewKit() {
  ...
  $('detail').innerHTML = `
    ...
    <div class="btnrow"><button onclick="buildKit()" id="kitBtn">🏗️ Build kit from website</button></div>
...
async function buildKit() {
  ...
  const btn = $('kitBtn'); btn.disabled = true; btn.innerHTML = '<span class="spin">🏗️</span> Reading site, extracting brand…';
  try {
  ...
  btn.disabled = false; btn.textContent = '🏗️ Build kit from website';
}
