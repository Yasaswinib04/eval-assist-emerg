...
const getGrade = (total, maxMarks) => {
  ...
  const grade = GRADES.find((g) => pct >= (g.min / 40) * 100) || GRADES[GRADES.length - 1];
  return grade;
};
...
const ScoreEntry = () => {
  ...
  const updateQuestion = useCallback((idx, field, value) => {
    setQuestions((prev) => {
      ...
      if (field === "chapter") {
        ...
        if (ch && ch.concepts.length > 0) {
          updated[idx].concept = ch.concepts[0];
        }
      }
      return updated;
    });
  ...
  const handlePasteFromClipboard = useCallback(async () => {
    try {
      ...
      if (parsedStudents.length > 0) {
        ...
        toast.error("No student data found. Use Tab-separated format: Name<Tab>Q1<Tab>Q2...");
      }
    } catch (err) {
  ...
  const getStudentGrade = useCallback((studentIdx) => {
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      students: students.map((s, i) => {
        ...
        const scoreMap = {};
        questions.forEach((q) => {
          scoreMap[String(q.number)] = studentScores[String(q.number)] || 0;
        });
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-8 md:py-12" data-testid="score-entry-page">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center gap-3 flex-wrap">
          <div className="flex-1 min-w-[200px]">
            ...
            <input
              ...
              placeholder="e.g. Unit Test 3 — Reproduction"
              className="w-full h-11 px-3 rounded-lg border border-stone-300 bg-white text-base focus:outline-none focus:ring-2 focus:ring-emerald-600"
          ...
          <div className="w-24">
            ...
            <input
              type="number"
              value={totalMarks}
              onChange={(e) => setTotalMarks(parseInt(e.target.value || "40", 10))}
              className="w-full h-11 px-3 rounded-lg border border-stone-300 bg-white text-base focus:outline-none focus:ring-2 focus:ring-emerald-600"
            />
          </div>
        </div>
        ...
        {questions.length === 0 && (
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                onClick={() => applyTemplate("10-mcq")}
                className="px-4 py-2 rounded-lg bg-stone-100 hover:bg-emerald-50 hover:text-emerald-800 border border-stone-200 hover:border-emerald-200 text-sm font-medium transition-colors"
              >
                10 MCQs (1 mark each)
              </button>
              <button
                onClick={() => applyTemplate("17-standard")}
                className="px-4 py-2 rounded-lg bg-stone-100 hover:bg-emerald-50 hover:text-emerald-800 border border-stone-200 hover:border-emerald-200 text-sm font-medium transition-colors"
              >
                17 Qs Standard (10 MCQ + 3 Short + 2 Long + 2 Essay)
              </button>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-wrap items-end gap-2 mb-4 p-3 bg-stone-50 rounded-lg border border-stone-200">
          <div>
            ...
            <input
              ...
              max="50"
              value={qCount}
              onChange={(e) => setQCount(parseInt(e.target.value || "1", 10))}
              className="w-16 h-9 px-2 rounded border border-stone-300 bg-white text-sm text-center focus:outline-none focus:ring-2 focus:ring-emerald-600"
            />
          </div>
          <div>
          ...
          <button
            ...
            className="h-9 px-4 rounded-lg bg-emerald-600 text-white text-sm font-medium hover:bg-emerald-700 transition-colors inline-flex items-center gap-1.5"
          >
            <Plus size={14} /> Add
          </button>
        </div>
                ...
                return (
                  <div key={i} className="grid grid-cols-[44px_64px_64px_1fr_1fr_44px] gap-1 px-3 py-1.5 items-center hover:bg-stone-50/50">
                    ...
                    <select
                      ...
                      className="h-8 px-1 rounded border border-stone-200 bg-white text-xs focus:outline-none focus:ring-1 focus:ring-emerald-500"
                    >
                      {SECTIONS.map((s) => <option key={s}>{s}</option>)}
                    </select>
                    <input
                      ...
                      step="0.5"
                      value={q.maxMarks}
                      onChange={(e) => updateQuestion(i, "maxMarks", parseFloat(e.target.value || "1"))}
                      className="h-8 w-full px-1 rounded border border-stone-200 bg-white text-xs text-center focus:outline-none focus:ring-1 focus:ring-emerald-500"
                    />
                    <select
                      value={q.chapter}
                      onChange={(e) => updateQuestion(i, "chapter", e.target.value)}
                      className="h-8 px-1 rounded border border-stone-200 bg-white text-xs focus:outline-none focus:ring-1 focus:ring-emerald-500"
                    >
                      {CHAPTERS.map((c) => <option key={c.id} value={c.id}>{c.name.split(":")[0]}</option>)}
                    </select>
                    <select
                      value={q.concept}
                      onChange={(e) => updateQuestion(i, "concept", e.target.value)}
                      className="h-8 px-1 rounded border border-stone-200 bg-white text-xs focus:outline-none focus:ring-1 focus:ring-emerald-500"
                    >
                    ...
                    <button
                      ...
                      className="h-7 w-7 rounded hover:bg-rose-50 text-stone-400 hover:text-rose-600 flex items-center justify-center"
                    >
                      <Trash2 size={14} />
                    </button>
                  </div>
        ...
        <div className="flex flex-wrap items-end gap-2 mb-3">
          <div className="flex-1 min-w-[280px]">
            ...
            <textarea
              ...
              placeholder="e.g. Karan Singh, Rahul Sharma, Aryan Patel&#10;Paste names from your register..."
              rows={2}
              className="w-full px-3 py-2 rounded-lg border border-stone-300 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-600 resize-none"
              data-testid="input-student-names"
            />
          </div>
          <button
            onClick={parseStudentNames}
            className="h-11 px-4 rounded-lg bg-emerald-600 text-white text-sm font-medium hover:bg-emerald-700 transition-colors inline-flex items-center gap-1.5"
          >
          ...
          <button
            ...
            className="h-11 px-4 rounded-lg bg-white border border-stone-300 text-stone-700 hover:bg-stone-50 text-sm font-medium inline-flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
            title={questions.length === 0 ? "Add questions first" : "Paste tab-separated data (Name, Q1, Q2, ...)"}
          >
          ...
          <div className="flex flex-wrap gap-2">
            {students.map((s, i) => (
              <div key={i} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-stone-100 border border-stone-200 text-sm">
                ...
                <button onClick={() => removeStudent(i)} className="h-5 w-5 rounded-full hover:bg-rose-100 text-stone-400 hover:text-rose-600 flex items-center justify-center">
                  <Trash2 size={10} />
                </button>
              </div>
            ))}
            {students.length > 0 && (
              <button
                onClick={() => { setStudents([]); setScores({}); }}
                className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-rose-50 border border-rose-200 text-rose-700 text-sm hover:bg-rose-100"
              >
                Clear all
              </button>
            )}
        ...
        <div className="bg-white border border-stone-200 rounded-xl shadow-sm mb-6 overflow-hidden">
          ...
          <div className="overflow-x-auto" ref={gridRef}>
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  <th className="sticky left-0 z-10 bg-stone-50 border-b border-r border-stone-200 px-3 py-2 text-left text-xs font-semibold text-stone-600 min-w-[140px]">
                    Student
                  </th>
                  {questions.map((q) => (
                    <th
                      key={q.number}
                      className="bg-stone-50 border-b border-r border-stone-100 px-2 py-2 text-center text-xs font-semibold text-stone-600 min-w-[60px]"
                      title={`${q.concept} (${q.maxMarks} marks)`}
                    >
                      <div>Q{q.number}</div>
                      <div className="text-[10px] text-stone-400 font-normal">{q.maxMarks}</div>
                    </th>
                  ))}
                  <th className="bg-stone-50 border-b border-stone-200 px-3 py-2 text-center text-xs font-semibold text-stone-600 min-w-[72px]">
                    Total
                  </th>
                  <th className="bg-stone-50 border-b border-stone-200 px-3 py-2 text-center text-xs font-semibold text-stone-600 min-w-[60px]">
                    Grade
                  </th>
                </tr>
              </thead>
              <tbody>
                {students.map((s, studentIdx) => {
                  ...
                  const grade = getStudentGrade(studentIdx);
                  return (
                    <tr key={studentIdx} className="hover:bg-stone-50/50 border-b border-stone-100">
                        ...
                        return (
                          <td key={q.number} className="border-r border-stone-50 px-0 py-0 text-center">
                            <div data-cell={`${studentIdx}-${q.number}`}>
                              <EditableCell
                                value={cellValue}
                                onChange={(val) => updateScore(studentIdx, q.number, val)}
                                onKeyDown={handleCellKey(studentIdx, q.number)}
                              />
                            </div>
                          </td>
                        );
                      ...
                      <td className="px-3 py-1.5 text-center">
                        <span className={`inline-flex items-center justify-center h-7 w-10 rounded-md text-xs font-bold ${
                          grade.color === "emerald" ? "bg-emerald-100 text-emerald-800" :
                          grade.color === "blue" ? "bg-blue-100 text-blue-800" :
                          grade.color === "amber" ? "bg-amber-100 text-amber-800" :
      ...
      <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
        <button
          ...
          className="h-12 px-5 rounded-lg bg-white border border-stone-300 text-stone-700 hover:bg-stone-50 font-medium"
        >
          Cancel
        </button>
        <button
          ...
          onClick={handleSave}
          className={`inline-flex items-center gap-2 h-12 px-6 rounded-lg font-medium text-white transition-colors ${
            canSave ? "bg-emerald-700 hover:bg-emerald-800" : "bg-stone-300 cursor-not-allowed"
          }`}
          data-testid="btn-save-scores"
        >
          {submitting ? (
            <><Loader2 size={18} className="animate-spin" /> Saving...</>
          ) : (
            <><BarChart3 size={18} /> Save & View Insights</>
          )}
        </button>
      </div>
    </div>
