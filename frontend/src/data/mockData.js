...
export const STUDENTS = [
  { id: "stu-01", name: "Aarav Sharma", roll: "08-01", total: 32, status: "done" },
  { id: "stu-02", name: "Bhavana Reddy", roll: "08-02", total: 28, status: "review" },
  { id: "stu-03", name: "Chetan Kumar", roll: "08-03", total: 35, status: "done" },
  { id: "stu-04", name: "Divya Lakshmi", roll: "08-04", total: 22, status: "review" },
  { id: "stu-05", name: "Esha Patel", roll: "08-05", total: 30, status: "review" },
  { id: "stu-06", name: "Farhan Khan", roll: "08-06", total: 18, status: "done" },
  { id: "stu-07", name: "Gowtham Rao", roll: "08-07", total: 26, status: "done" },
  { id: "stu-08", name: "Hima Bindu", roll: "08-08", total: 31, status: "review" },
];

export const EVALUATIONS = {
  "stu-02": [
    { qId: "q1", studentAnswer: "C) Hen", aiMark: 0, confidence: "high", confidenceScore: 96, needsReview: false, reasoning: "Student selected 'Hen' but correct answer is 'Humans' — humans show internal fertilization while others are external. Standard MCQ scoring." },
    { qId: "q2", studentAnswer: "C", aiMark: 1, confidence: "high", confidenceScore: 98, needsReview: false, reasoning: "Correct option. IVF is performed when oviducts are blocked." },
    ...
    { qId: "q4", studentAnswer: "D", aiMark: 1, confidence: "high", confidenceScore: 97, needsReview: false, reasoning: "Correct. Wheat needs cool and dry weather — Rabi crop." },
    { qId: "q5", studentAnswer: "B", aiMark: 1, confidence: "high", confidenceScore: 95, needsReview: false, reasoning: "Correct. Both frog and butterfly undergo metamorphosis." },
    { qId: "q6", studentAnswer: "Amoeba", aiMark: 1, confidence: "medium", confidenceScore: 67, needsReview: true, reasoning: "Handwriting unclear — looks like 'Amoeba' but 'a' loops resemble 'o'. Verify the diagram-based answer." },
    { qId: "q7", studentAnswer: "A", aiMark: 0, confidence: "high", confidenceScore: 94, needsReview: false, reasoning: "Incorrect. Combine is for harvesting, not ploughing → Assertion false, Reason true → option D." },
    { qId: "q8", studentAnswer: "A", aiMark: 1, confidence: "high", confidenceScore: 98, needsReview: false, reasoning: "Correct. Drip irrigation conserves water during drought." },
    { qId: "q9", studentAnswer: "C", aiMark: 1, confidence: "high", confidenceScore: 96, needsReview: false, reasoning: "Correct. Cloning is asexual; statement C is the incorrect one." },
    { qId: "q10", studentAnswer: "B", aiMark: 1, confidence: "high", confidenceScore: 97, needsReview: false, reasoning: "Correct. Market eggs are not fertilised." },
    { qId: "q11", studentAnswer: "Unwanted plants are weeds. We remove them by hand and use chemicals.", aiMark: 1.5, confidence: "medium", confidenceScore: 71, needsReview: true, reasoning: "Definition correct (1). Mentioned manual removal and chemicals (0.5). Did not name 'weedicide' or examples. Consider awarding 2 if hand-pulling counts in your rubric." },
    { qId: "q12", studentAnswer: "No it is not correct. Antibiotics should be taken only when doctor says. Finish all tablets.", aiMark: 2, confidence: "high", confidenceScore: 92, needsReview: false, reasoning: "Both parts addressed: incorrectness + two precautions. Full marks." },
    { qId: "q13", studentAnswer: "Egg is big and round. Sperm is small and has a tail to swim.", aiMark: 1.5, confidence: "medium", confidenceScore: 73, needsReview: true, reasoning: "Two valid differences (size + motility). Acceptable for Class 8 but lacks origin point. Award 2 if conversational language is OK." },
    { qId: "q14", studentAnswer: "Salt — fish. Sugar — jam. Oil — pickle. Drying — papad.", aiMark: 4, confidence: "high", confidenceScore: 95, needsReview: false, reasoning: "All four methods named with valid Indian household examples. Full marks." },
    { qId: "q15", studentAnswer: "[Diagram drawn]", aiMark: 2.5, confidence: "low", confidenceScore: 54, needsReview: true, reasoning: "Diagram present; ovary, uterus, vagina labelled. Fallopian-tube label unclear in scan. Manual review of drawing quality recommended." },
    { qId: "q16", studentAnswer: "i) Virus  ii) Tuberculosis  iii) Use mosquito nets and clean surroundings  iv) Air/Contact", aiMark: 7, confidence: "high", confidenceScore: 91, needsReview: false, reasoning: "All four sub-parts correct (2 marks each). Brevity on (iii) — could mention insecticides but core concept present." },
    { qId: "q17", studentAnswer: "Asexual reproduction means one parent only. Budding in Hydra — small bud grows and falls off. Binary fission in Amoeba — one becomes two.", aiMark: 6, confidence: "medium", confidenceScore: 78, needsReview: true, reasoning: "Definition (2/2), Budding (2/2), Binary fission (2/4 — missing nucleus-division detail). Consider 7-8 if rubric allows Class 8 simplification." },
  ],
};
...
export const CHAPTER_PERFORMANCE = [
...
export const ROOT_CAUSE_INSIGHTS = [
...
export const INTERVENTION_LIST = [
  { studentId: "stu-06", name: "Farhan Khan", roll: "08-06", score: 18, weakConcepts: ["IVF", "Metamorphosis", "Female Reproductive System"], priority: "high" },
  { studentId: "stu-04", name: "Divya Lakshmi", roll: "08-04", score: 22, weakConcepts: ["Fertilization", "IVF", "Agricultural Implements"], priority: "high" },
  { studentId: "stu-07", name: "Gowtham Rao", roll: "08-07", score: 26, weakConcepts: ["Metamorphosis", "Asexual Reproduction"], priority: "medium" },
  { studentId: "stu-02", name: "Bhavana Reddy", roll: "08-02", score: 28, weakConcepts: ["Female Reproductive System"], priority: "low" },
];
...
export const getAssessment = (id) => ASSESSMENTS.find((a) => a.id === id) || ASSESSMENTS[0];
export const getEvaluationForStudent = (studentId) => EVALUATIONS[studentId] || EVALUATIONS["stu-02"];
export const getStudentProfile = (studentId) => STUDENT_PROFILES[studentId] || STUDENT_PROFILES["stu-02"];

// ── Cross-assessment trend data (Term 2: Nov 2025 – Feb 2026) ──
// Score timelines per student across 4 assessments (oldest → newest)
export const STUDENT_TERM_TRENDS = {
  "stu-02": [
...
export const STUDENT_CONCEPT_TRENDS = {
  "stu-02": [
...
export const getStudentTermTrend = (studentId) => STUDENT_TERM_TRENDS[studentId] || STUDENT_TERM_TRENDS["stu-02"];
export const getStudentConceptTrend = (studentId) => STUDENT_CONCEPT_TRENDS[studentId] || STUDENT_CONCEPT_TRENDS["stu-02"];
