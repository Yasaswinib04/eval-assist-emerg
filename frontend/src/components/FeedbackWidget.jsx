...
const FeedbackWidget = () => {
  ...
  return (
    <>
      {open && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-end justify-end p-4 sm:p-6" onClick={() => setOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl border border-stone-200 p-5 w-full max-w-sm relative" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="font-medium text-stone-900 mb-1">Facing a problem? Tell us</div>
            <p className="text-xs text-stone-500 mb-3">If something is not working, confusing, or you have a suggestion — write it here. We read every message.</p>
            {sent ? (
      ...
      {!open && (
        <button onClick={() => setOpen(true)} className="fixed bottom-5 right-5 z-40 h-11 pl-3 pr-4 rounded-full bg-blue-800 text-white shadow-lg hover:bg-blue-900 transition-colors flex items-center gap-2 text-sm font-medium" style={{marginBottom: 'env(safe-area-inset-bottom)'}}>
          <MessageSquare size={17} />
          <span>Problem? Tell us</span>
        </button>
      )}
    </>
  );
